...
def get_verify_data(ts_code, launch_date):
    ...
    Returns: {
        ...
        'is_overheat': bool, 'overheat_reason': str,
    ...
    result = {
        ...
        'is_overheat': False, 'overheat_reason': '',
        ...
        if len(mdf) >= 4:
            ...
            if len(prev) >= 3:
                ...
                if not ddf.empty:
                    ...
                    if result['pre_1m'] > PRE_1M_CHG_THRESH:
                        result['is_overheat'] = True
                        result['overheat_reason'] = f"1月{result['pre_1m']:+.1f}%>{PRE_1M_CHG_THRESH:.0f}%"
...
def print_verify_summary(scan_date_str, candidates):
    ...
    print(f"{'='*90}")
    print(f"{'代码':<12} {'象限':<6} {'分':<3} {'月线过热':<12} {'正天数':<8} {'5D增幅':<10} {'30D净买':<12} {'5D/30D':<8} {'峰值':<8} {'拐点':<10} {'类型':<10}")
    print(f"{'-'*90}")

    for ts_code, launch_date, quadrant, score in candidates:
        v = get_verify_data(ts_code, launch_date)
        overheat_str = v['overheat_reason'] if v['is_overheat'] else ('✓通过' if v['pre_1m'] != 0 else '无数据')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='史诗级行情扫描器 v2.9')
