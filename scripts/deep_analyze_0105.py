...
launch_date = "20260105"

for code, name in [("688416.SH", "鼎通科技"), ("300082.SZ", "奥克股份")]:
    print(f"\n{'='*60}")
    ...
    for _, row in monthly.tail(4).iterrows():
        print(f"  {row['trade_date']}: 收盘={row['close']}, MA5={row['ma5']:.2f}, MA10={row['ma10']:.2f}, MA20={row['ma20']:.2f}")
    
    # 计算月线MA5方向
    if len(monthly) >= 3:
        m_current = monthly.iloc[-1]
        m_prev = monthly.iloc[-2]
        m_prev2 = monthly.iloc[-3]
        ma5_up = m_current['ma5'] > m_prev['ma5']
        ma5_up_prev = m_prev['ma5'] > m_prev2['ma5']
        print(f"月线MA5方向: {'上升' if ma5_up else '下降'} (当期{m_current['ma5']:.2f} vs 上期{m_prev['ma5']:.2f})")
        print(f"  604 MA5={m_prev['ma5']:.2f}, 603收盘={m_prev['close']} {'✓站上MA5' if m_prev['close'] > m_prev['ma5'] else '✗低于MA5'}")
        print(f"  605 MA5={m_current['ma5']:.2f}, 605收盘={m_current['close']} {'✓站上MA5' if m_current['close'] > m_current['ma5'] else '✗低于MA5'}")
    ...
    # 启动日融资变化
    if len(pre5_margin) > 0 and len(ld_m) > 0:
        prev_row = pre5_margin.iloc[-1]
        if prev_row["rzye"] > 0:
            margin_chg = (ld_m.iloc[0]["rzye"] / prev_row["rzye"] - 1) * 100
